...
function deduplicateLinks(extractedLinks) {
    const map = new Map();
...
async function collectLinksFromHomepage() {
    ...
    try {
        ...
        const extractedLinks = await stagehand.extract("extract all links on the page with their link text", z.object({
        ...
        console.log(`All links on the page (${extractedLinks.links.length} total, ${uniqueLinks.length} unique):`);
 ...
 */
async function verifySingleLink(link) {
    ...
    try {
        ...
        const verification = await browser.extract(`Does the page content match what the link text "${link.linkText}" suggests? Extract the page title and provide a brief assessment (maximum 8 words).`, z.object({
            pageTitle: z.string(),
            contentMatches: z.boolean(),
            assessment: z.string(),
        }));
        console.log(`[${link.linkText}] Page Title: ${verification.pageTitle}`);
        console.log(`[${link.linkText}] Content Matches: ${verification.contentMatches ? "YES" : "NO"}`);
 ...
 */
async function verifyLinksInBatches(links) {
 ...
 */
function outputResults(results, label = "FINAL RESULTS") {
    console.log("\n" + "=".repeat(80));
    console.log(label);
