...
async function main() {
    ...
    try {
        ...
        const formFields = await stagehand.observe("Find form fields for: first name, last name, company, job title, email, message");
        // Execute all actions without LLM calls
        for (const field of formFields) {
            // Match field to data based on description
            let value = "";
            const desc = field.description.toLowerCase();
            if (desc.includes("first name"))
                value = firstName;
            else if (desc.includes("last name"))
                value = lastName;
            else if (desc.includes("company"))
                value = company;
            else if (desc.includes("job title"))
                value = jobTitle;
            else if (desc.includes("email"))
                value = email;
            else if (desc.includes("message"))
                value = message;
            if (value) {
                await stagehand.act({
                    ...field,
