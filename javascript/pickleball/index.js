...
import { z } from "zod";
async function loginToSite(stagehand, email, password) {
...
async function selectFilters(stagehand, activity, timeOfDay, selectedDate) {
...
async function checkAndExtractCourts(stagehand, timeOfDay) {
    ...
    const courtData = await stagehand.extract("Extract all available court booking information including court names, time slots, locations, and any other relevant details", z.object({
    ...
    let hasAvailableCourts = courtData.courts.some((court) => !court.availability.toLowerCase().includes("no free spots") &&
    ...
    if (availableCourts.length === 0 || !hasAvailableCourts) {
        ...
        const alternativeTimes = timeOfDay === "Morning"
            ? ["Afternoon", "Evening"]
            : timeOfDay === "Afternoon"
                ? ["Morning", "Evening"]
                : ["Morning", "Afternoon"];
        for (const altTime of alternativeTimes) {
            ...
            if (altAvailableCourts.length > 0) {
                const altCourtData = await stagehand.extract("Extract all available court booking information including court names, time slots, locations, and any other relevant details", z.object({
                    courts: z.array(z.object({
                        name: z.string().describe("the name or identifier of the court"),
                        openingTimes: z
                            .string()
                ...
                const hasAltAvailableCourts = altCourtData.courts.some((court) => !court.availability.toLowerCase().includes("no free spots") &&
                    !court.availability.toLowerCase().includes("unavailable") &&
                    !court.availability.toLowerCase().includes("next available") &&
                    !court.availability.toLowerCase().includes("the next available reservation"));
    ...
    if (!hasAvailableCourts) {
        ...
        const finalCourtData = await stagehand.extract("Extract all available court booking information including court names, time slots, locations, and any other relevant details", z.object({
            courts: z.array(z.object({
                name: z.string().describe("the name or identifier of the court"),
                openingTimes: z.string().describe("the opening hours or operating times of the court"),
                location: z.string().describe("the location or facility name"),
...
async function bookCourt(stagehand) {
    ...
    try {
        ...
        const confirmation = await stagehand.extract("Extract any booking confirmation message, success notification, or reservation details", z.object({
...
async function selectTimeOfDay() {
