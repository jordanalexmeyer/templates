...
async function getWeatherForLocation(geolocation) {
    ...
    try {
        ...
        const extractResult = await stagehand.extract("Extract the current temperature and its unit", z.object({
            temperature: z.number().describe("The current temperature value"),
            unit: z.string().describe("The temperature unit)"),
        }));
        console.log(`Successfully extracted weather data for ${cityName}: ${extractResult.temperature} ${extractResult.unit}`);
