...
async function saveDownloadsWithRetry(bb, sessionId, timeoutSecs = 60) {
...
function extractFilesFromZip(zipPath, outputDir = "output/documents") {
...
async function parseReceiptsWithExtend(filePaths) {
    ...
    for (let i = 0; i < filePaths.length; i += 9) {
        ...
        const batchResults = await Promise.all(batch.map(async (filePath) => {
            ...
            try {
                ...
                const uploadResponse = await client.files.upload(blob, { maxRetries: 4 });
    ...
    const csvRows = [];
    csvRows.push("file,vendor_name,receipt_date,receipt_number,total_amount,currency,subtotal,tax,payment_method,line_items_count");
...
async function main() {
    ...
    try {
        ...
        const downloadButtons = await stagehand.observe("Find all the small Download links on individual receipt cards.");
...
main().catch((err) => {
    ...
    console.error("  - Check .env file has BROWSERBASE_PROJECT_ID, BROWSERBASE_API_KEY, and GOOGLE_API_KEY");
