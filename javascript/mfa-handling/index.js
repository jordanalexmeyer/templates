...
async function main() {
    ...
    try {
        ...
        const credentials = await stagehand.extract("Extract the test email, password, and TOTP secret key shown on the page", z.object({
            email: z.string(),
            password: z.string(),
        ...
        try {
            ...
            console.warn("Timed out waiting for 'networkidle' after submit; continuing because the login likely succeeded.", err);
        }
        // Check if login succeeded
        console.log("Checking authentication result...");
        const result = await stagehand.extract("Check if the login was successful or if there's an error message", z.object({
            success: z.boolean(),
            message: z.string(),
        }));
        if (result.success) {
            ...
            try {
                ...
                console.warn("Timed out waiting for 'networkidle' after retry submit; continuing because the login likely succeeded.", err);
            }
