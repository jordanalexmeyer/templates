 ...
 */
async function generateOneLiner(domain) {
    ...
    try {
        ...
        const valueProp = await stagehand.extract("extract the value proposition from the landing page", z.object({
            value_prop: z.string(),
        }));
        console.log(`📊 Extracted value prop for ${domain}:`, valueProp.value_prop);
        // Validate extraction returned meaningful content
        if (!valueProp.value_prop ||
...
async function main() {
    ...
    try {
        ...
        console.error(`\n❌ Error: ${errorMessage}`);
        console.error("\nCommon issues:");
        console.error("  - Check .env file has BROWSERBASE_API_KEY set (required for browser automation)");
