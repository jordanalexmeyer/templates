...
async function generateSearchQueries(recipient, description) {
...
async function scoreProducts(products, recipient, description) {
    ...
    const productList = allProducts
        .map((product, index) => `${index + 1}. ${product.title} - ${product.price} - ${product.rating}`)
        .join("\n");
...
- Use productIndex 1-${allProducts.length}`,
    ...
    try {
        ...
        const scoredProducts = allProducts.map((product, index) => {
            const scoreInfo = scoresData.find((s) => s.productIndex === index + 1);
            return {
                ...product,
...
async function main() {
    ...
    async function runSingleSearch(query, sessionIndex) {
        ...
        try {
            ...
            const productsData = await sessionStagehand.extract("Extract the first 3 products from the search results", z.object({
                products: z
                    .array(z.object({
                    ...
                    rating: z
                        .string()
                        .describe("the star rating or number of reviews (e.g., '4.5 stars' or '123 reviews')"),
                }))
                    .max(3)
                    .describe("array of the first 3 products from search results"),
            }));
            console.log(`Session ${sessionIndex + 1}: Found ${productsData.products.length} products for "${query}"`);
