...
function saveDownloadsWithRetry(bb, sessionId, retryForSeconds = 30) {
...
function extractPdfFromZip(zipPath, outputDir = "downloaded_files") {
...
async function extractPDFWithReducto(pdfPath, reductoaiClient) {
...
async function main() {
    ...
    try {
        ...
        const { promise: downloadPromise, stopPolling } = saveDownloadsWithRetry(bb, stagehand.browserbaseSessionId, 45);
        try {
...
main().catch((err) => {
    ...
    console.error("  - Check .env file has BROWSERBASE_PROJECT_ID, BROWSERBASE_API_KEY, and REDUCTOAI_API_KEY");
