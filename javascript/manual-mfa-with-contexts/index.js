...
async function createSessionWithContext() {
    ...
    const mfaRequired = await stagehand.extract("Is there a two-factor authentication or verification code prompt on the page?", z.boolean());
    if (mfaRequired) {
        console.log("MFA DETECTED!");
        console.log("═══════════════════════════════════════════════════════════");
        console.log("PAUSED: Please complete MFA in the browser");
        console.log("═══════════════════════════════════════════════════════════");
        console.log(`1. Open the Browserbase session in your browser: https://browserbase.com/sessions/${stagehand.browserbaseSessionID}`);
 ...
 */
async function reuseContext(contextId) {
    ...
    const username = await stagehand.extract("Extract the logged-in username or check if we're authenticated", z.string());
...
async function deleteContext(contextId) {
    ...
    try {
        ...
        console.log(`Could not delete context: ${error instanceof Error ? error.message : String(error)}`);
        console.log("   Context will auto-expire after 30 days\n");
    }
}
async function main() {
