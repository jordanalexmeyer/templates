...
async function withRetry(fn, description, maxRetries = 3, delayMs = 2000) {
    let lastError = null;
    ...
    try {
      ...
      Do not ask follow up questions, the user will trust your judgement.`,
        ...
        const [termsResult, privacyResult] = await Promise.allSettled([
            stagehand.extract("extract the link to the Terms of Service page (may also be labeled as Terms of Use, Terms and Conditions, or similar equivalent names)", z.object({
                termsOfServiceLink: z.string().url(),
            })),
            stagehand.extract("extract the link to the Privacy Policy page (may also be labeled as Privacy Notice, Privacy Statement, or similar equivalent names)", z.object({
        ...
        if (termsOfServiceLink) {
            ...
            try {
                const addressResult = await stagehand.extract("Extract the physical company mailing address (street, city, state, postal code, and country if present) from the Terms of Service page. Ignore phone numbers or email addresses.", z.object({
        ...
        if (!address && privacyPolicyLink) {
            ...
            try {
                const addressResult = await stagehand.extract("Extract the physical company mailing  address(street, city, state, postal code, and country if present) from the Privacy Policy page. Ignore phone numbers or email addresses.", z.object({
                    companyAddress: z.string(),
                }));
                const companyAddress = addressResult.companyAddress || "";
                if (companyAddress && companyAddress.trim().length > 0) {
