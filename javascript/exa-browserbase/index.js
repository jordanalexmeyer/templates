...
function buildAgentInstruction(jobDescription) {
...
async function uploadResume(stagehand, logPrefix = "") {
...
async function applyToJob(careersPage, index) {
    ...
    try {
        ...
        const jobDescription = await stagehand.extract("extract the full job description including title, requirements, responsibilities, and any important details about the role", jobDescriptionSchema);
...
async function main() {
    ...
    console.log(`\n${"=".repeat(50)}`);
    console.log(`Starting applications (${searchConfig.concurrent ? `concurrent, max ${searchConfig.maxConcurrentBrowsers} browsers` : "sequential"})...`);
    ...
    if (searchConfig.concurrent) {
        ...
        for (const chunk of chunks) {
            const chunkResults = await Promise.all(chunk.map((page, idx) => applyToJob(page, results.length + idx)));
...
main().catch((err) => {
    ...
    console.error("  - Check .env file has BROWSERBASE_PROJECT_ID, BROWSERBASE_API_KEY, MODEL_API_KEY, and EXA_API_KEY");
