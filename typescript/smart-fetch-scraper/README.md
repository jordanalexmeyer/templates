# Browserbase: Smart Fetch Scraper
...
- Browser fallback: when the Fetch API returns insufficient or JS-rendered content, automatically falls back to a Playwright browser session via Browserbase CDP.
...
- Fetch API: Browserbase's lightweight HTTP fetching endpoint — fetches page content through Browserbase infrastructure without spinning up a browser.
  Docs → https://docs.browserbase.com/features/fetch
- CDP: Chrome DevTools Protocol — used to connect Playwright to a Browserbase cloud browser session.
  Docs → https://docs.browserbase.com/sdk/node-sdk
...
- On Fetch API success: prints page title, link count, content length, and a 500-char preview
- On browser fallback: prints Browserbase live view link, then the same summary and preview from the rendered HTML
...
• Hybrid data extraction: quick HTML parsing for simple pages, full rendering for JS-heavy ones.
• Monitoring and alerting: cheaply poll pages for content changes, escalate to browser only when needed.

## NEXT STEPS

• Add a proper HTML parser (e.g., cheerio) for richer Fetch API-side extraction.
• Batch multiple URLs and fan out Fetch API calls concurrently (up to your concurrency limit).
• Add caching: hash Fetch API responses to detect changes before running expensive browser sessions.
...
📚 Browserbase Node SDK: https://docs.browserbase.com/sdk/node-sdk
